...
## 방법 0: Render 원클릭 클라우드 배포 (서버 없이 지금 바로 링크 받기)

사내 서버를 준비하기 전, 무료 클라우드로 즉시 공개 링크를 만들 수 있습니다.

1. https://render.com 가입 (GitHub 계정으로 로그인)
2. **New → Blueprint** → 이 GitHub 저장소(`limjh0301/Main-Project`) 연결
   - 브랜치를 `claude/ocr-text-extraction-app-ceswc0`로 선택
3. **Apply** → 몇 분 후 `https://yogu-xxxx.onrender.com` 형태의 링크가 생성됩니다.

특징과 유의점:
- OCR 포함 전체 기능이 Docker 이미지로 배포됩니다 (`render.yaml`이 자동 인식됨).
- 무료 플랜은 15분간 접속이 없으면 잠들었다가 첫 접속 시 1분쯤 걸려 깨어납니다.
- **외부(미국) 클라우드를 경유하므로**, 업로드 파일이 서버에 저장되지는 않지만
  민감한 요구서를 다루는 기관 정책상 허용되는지 먼저 확인하세요.
  정식 운영은 아래 사내 서버 방식을 권장합니다.
